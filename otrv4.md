...
> REMARK there is currently reasoning that one client (with specifically: instance tag + versions) has a client-profile. However, there are a few downsides to this: (1.) instance tags cannot vary flexibly as needed; (2.) there is significant maintenance overhead in maintaining public published profiles for each client; (3.) the instance-tag easily changes so does little to "prove"/identify the client itself and private key identifies the user. (4.) OTR may be used over different networks/protocols, thus a persistent instance tag may hint/identify client over different networks.
> Instead: one might consider a design where "instance-tag" is dropped, "versions" represents the "lower bound" acceptable supported versions (over all clients) and a client-profile published for a user instead of a single client. (That way, instance-tags can easily be varied.) OR said otherwise: I have yet to discover why there is a _requirement_ that client-profile should be tied to a single client, apart from broadcasting precise capabilities, i.e. "protocol versions". (Additionally, the instance tag should only be significant when several clients all respond to an OTR query, to distinguish between clients. Some accounts have instance-identifiers, sometimes only one active session is allowed at a time thus instance-tag is not relevant. Making it part of the client-profile elevates the intended purpose/importance of the instance-tag. That is, instance tags only distinguish between multiple active clients on the same account.)
